...
`init` imports the union of `~/.pi/agent/intelligence-profiles/*.json`, deduplicating model/effort pairs and preserving role, fit, character and profile provenance. These maps are guidance, not an allowlist: edit the router's candidate catalog to add any supported model. Initial priors are neutral `0.7`; the ACTIVE guide only supplies a deterministic tie-break rank. Subsequent profile switches do not rewrite router policy. No model aliases or benchmark mappings are inferred. Native Codex `max` and native Cursor transport are excluded.
